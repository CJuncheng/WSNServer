        ...
        where TRequestInfo : IRequestInfo
    {
        ...
        public override TRequestInfo Filter(byte[] readBuffer, int offset, int length, bool toBeCopied, out int rest)
        {
            ...
            while (true)
            {
                ...
                {
                    //以下属于修改supersocket源代码以实现协议中数据可能出现 AA AA的情况解析
                    //数据实际的长度 这里主要应对帧中包含数据AA AA 而不是结束 AA AA的情况
                    var dataLength = commandData.Length;
                    var protolDataLength = (int)commandData[2];

                    if ((dataLength - 4) >= protolDataLength)
                    {
                        Reset();
                        return requestInfo;
                    }
                    ...
                    {
                        //数据中可能出现AA AA 则直接查找下一个AA AA 这里留空 supersocket filter会自动查找下一个AA AA
                    }
                    //以上属于修改supersocket源代码以实现协议中数据可能出现 AA AA的情况解析
